...
PlayFreeKickOur::PlayFreeKickOur(WorldModel *wm, StrategyResult *sr, Knowledge *kn, QObject *parent) :
    Play(wm, sr, kn, "PlayFreeKickOur", parent)
{
    ...
    _td3 = new TacticDefend(wm, kn, this);
    _rd = new RoleDefender(this);

    _ta = new TacticAttack(wm, kn, this);

    _tb = new TacticBlock(wm, kn, this);
...
bool PlayFreeKickOur::EnterCondition()
{
    if(_wm->cmgs.ourFreeKick() || _wm->cmgs.ourIndirectKick())
        return true;
    else
        return false;
}

bool PlayFreeKickOur::ExecutePlayEngine()
{
    ...
    if(agents.count()==4)
        ...
        _ts1->setTotalStop(3);
    _ts1->setMyStop(1);
    _ts1->setRID(agents[4]);
    _ts1->setTarget(_wm->ball.pos.loc,Vector2D(-3010,0),500,true);

    _sr->tactics[agents[4]] = _ts1;
