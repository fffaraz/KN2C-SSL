...
PlayFreeKickOpp::PlayFreeKickOpp(WorldModel *wm, StrategyResult *sr, Knowledge *kn, QObject *parent) :
    Play(wm, sr, kn, "PlayFreeKickOpp", parent)
{
...
bool PlayFreeKickOpp::EnterCondition()
{
    if(_wm->cmgs.theiFreeKick() || _wm->cmgs.theirDirectKick())
        return true;
    else
        return false;
}

bool PlayFreeKickOpp::ExecutePlayEngine()
{
